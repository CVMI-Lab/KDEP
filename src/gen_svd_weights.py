...
parser.add_argument('--KD_b', type=float, default=0.0, help='Balancing weight between losses in KD.')
...
parser.add_argument('--clip_downstream_add_text_emb', action='store_true',help='Whether to add text embed, only for FKD from clip')
...
def main(args, store):
    ...
    if args.teacher_path:
        ...
        get_svd(teacher_model, train_loader)
...
def get_svd(res50, loader):
    ...
    for i, (inp, target) in iterator:
        ...
        (_,feat), _ = res50(inp) # feat from sh
    ...
    VT = vh.numpy()
    np.save('pretrained-models/model_v.npy', VT)
    np.save('pretrained-models/model_feat_mean.npy', feat_mean.numpy())
    np.save('pretrained-models/model_eigen_values.npy', ff.numpy())
