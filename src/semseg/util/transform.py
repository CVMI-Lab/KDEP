...
class PhotoMetricDistortion(object):
    """Apply photometric distortion to image sequentially, every transformation
    is applied with a probability of 0.5. The position of np_random contrast is in
    second or second to last.
    1. np_random brightness
    2. np_random contrast (mode 0)
    3. convert color from BGR to HSV
    4. np_random saturation
    5. np_random hue
    6. convert color from HSV to BGR
    7. np_random contrast (mode 1)
    Args:
        brightness_delta (int): delta of brightness.
        contrast_range (tuple): range of contrast.
        saturation_range (tuple): range of saturation.
    ...
    def brightness(self, img):
        ...
        if np_random.randint(2):
            return self.convert(
                img,
                beta=np_random.uniform(-self.brightness_delta,
                                    self.brightness_delta))
        return img
    ...
    def contrast(self, img):
        ...
        if np_random.randint(2):
            return self.convert(
                img,
                alpha=np_random.uniform(self.contrast_lower, self.contrast_upper))
        return img
    ...
    def saturation(self, img):
        ...
        if np_random.randint(2):
            ...
            img[:, :, 1] = self.convert(
                img[:, :, 1],
                alpha=np_random.uniform(self.saturation_lower,
                                     self.saturation_upper))
            # img = mmcv.hsv2bgr(img)
            img = cv2.cvtColor(img, cv2.COLOR_HSV2BGR)
        return img
    ...
    def hue(self, img):
        ...
        if np_random.randint(2):
            ...
            img[:, :,
                0] = (img[:, :, 0].astype(int) +
                      np_random.randint(-self.hue_delta, self.hue_delta)) % 180
            # img = mmcv.hsv2bgr(img)
            img = cv2.cvtColor(img, cv2.COLOR_HSV2BGR)
        return img
    ...
    def __call__(self, image, label=None):
        ...
        img = image
        # np_random brightness
        img = self.brightness(img)

        # mode == 0 --> do np_random contrast first
        # mode == 1 --> do np_random contrast last
        mode = np_random.randint(2)
        if mode == 1:
            img = self.contrast(img)

        # np_random saturation
        img = self.saturation(img)
