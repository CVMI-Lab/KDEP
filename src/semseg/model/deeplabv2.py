...
class ResNetDeepLab(nn.Module):
    ...
    def __init__(self, block, layers, num_classes, mean, std, criterion=nn.CrossEntropyLoss(ignore_index=255)):
        self.MEAN = mean
        self.STD = std
        self.inplanes = 64
...
def Resnet101_deeplab(num_classes=21, criterion=nn.CrossEntropyLoss(ignore_index=255), pretrained=True, pretrain_data='?'):
    ...
    model = ResNetDeepLab(Bottleneck, [3, 4, 23, 3], num_classes, mean, std, criterion=criterion)
    if pretrained:
        # saved_state_dict = load_state_dict_from_url(_RESNET_101_DEEPLAB_COCO_URL)
        if pretrain_data == 'coco':
            model_path = './initmodel/deeplabv2/resnet101COCO-41f33a49.pth'
            print('loaded deeplabv2 resnet101 coco weights')
        elif pretrain_data == 'imagenet':
            model_path = './initmodel/deeplabv2/resnet101-5d3b4d8f.pth'
