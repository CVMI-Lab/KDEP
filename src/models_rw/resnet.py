...
class Bottleneck_pre_relu(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, cardinality=1, base_width=64,
                 reduce_first=1, dilation=1, first_dilation=None, act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d,
                 attn_layer=None, aa_layer=None, drop_block=None, drop_path=None, pre_relu=False):
...
def make_blocks(
        ...
        down_kernel_size=1, avg_down=False, drop_block_rate=0., drop_path_rate=0., last_pre_relu=False, **kwargs):
    stages = []
    feature_info = []
    ...
    for stage_idx, (planes, num_blocks, db) in enumerate(zip(channels, block_repeats, drop_blocks(drop_block_rate))):
        ...
        for block_idx in range(num_blocks):
            ...
            if last_pre_relu and stage_idx == 3 and block_idx == num_blocks-1:
                blocks.append(block_fn(
                    inplanes, planes, stride, downsample, first_dilation=prev_dilation,
                    drop_path=DropPath(block_dpr) if block_dpr > 0. else None, pre_relu=True, **block_kwargs))
            else:
                blocks.append(block_fn(
                    inplanes, planes, stride, downsample, first_dilation=prev_dilation,
...
class ResNet_pre_relu(nn.Module):
    """ResNet / ResNeXt / SE-ResNeXt / SE-Net
    ...
    def __init__(self, block, layers, num_classes=1000, in_chans=3,
        ...
        self.num_classes = num_classes
        self.drop_rate = drop_rate
        super(ResNet_pre_relu, self).__init__()
        ...
        stage_modules, stage_feature_info = make_blocks(
            ...
            output_stride=output_stride, reduce_first=block_reduce_first, avg_down=avg_down,
            down_kernel_size=down_kernel_size, act_layer=act_layer, norm_layer=norm_layer, aa_layer=aa_layer,
            drop_block_rate=drop_block_rate, drop_path_rate=drop_path_rate, last_pre_relu=True, **block_args)
        for stage in stage_modules:
            self.add_module(*stage)  # layer1, layer2, etc
        self.feature_info.extend(stage_feature_info)
        ...
        import numpy as np
        self.pca_v = np.load(
            'pretrained-models/V_WRN50_100k_subset_from_train_loader_rm-mean_pre_relu_no_random.npy')
        self.pca_mean = np.load(
            'pretrained-models/V_WRN50_100k_subset_from_train_loader_rm-mean_pre_relu_feat_mean_no_random.npy')
        self.pca_v = torch.from_numpy(self.pca_v)
    ...
    def forward(self, x):
        ...
        x = self.global_pool(x)
        feat = x.view(x.size(0), -1)
        feat = feat - self.pca_mean.cuda()
        feat = torch.matmul(feat, self.pca_v[:, :512].cuda())  # B 512
...
def _create_resnet_pre_relu(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(
        ResNet_pre_relu, variant, pretrained,
        default_cfg=default_cfgs[variant],
        **kwargs)
...
@register_model
def wide_resnet50_2_pre_relu_pca(pretrained=False, **kwargs):
    """Constructs a Wide ResNet-50-2 model.
    The model is the same as ResNet except for the bottleneck number of channels
    which is twice larger in every block. The number of channels in outer 1x1
    convolutions is the same, e.g. last block in ResNet-50 has 2048-512-2048
    channels, and in Wide ResNet-50-2 has 2048-1024-2048.
    """
    model_args = dict(block=Bottleneck_pre_relu, layers=[3, 4, 6, 3], base_width=128, **kwargs)
    return _create_resnet_pre_relu('wide_resnet50_2', pretrained, **model_args)
