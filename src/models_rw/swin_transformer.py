...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        import numpy as np
        # self.pca_v = np.load('pretrained-models/V-swin-s-128k_from_train_loader_rm-mean_pre_relu_no_random.npy')
        self.pca_v = np.load('pretrained-models/V-swin-b-128k_from_train_loader_rm-mean_pre_relu_no_random.npy')

        # self.pca_mean = np.load('pretrained-models/V-swin-s-128k_from_train_loader_rm-mean_pre_relu_feat_mean_no_random.npy')
        self.pca_mean = np.load('pretrained-models/V-swin-b-128k_from_train_loader_rm-mean_pre_relu_feat_mean_no_random.npy')
    ...
    def forward(self, x):
        with torch.no_grad():
            ...
            # feat += 6
            # assert (feat < 0).sum() == 0, "offset not enough"
            x = self.head(x)
            return x, feat
